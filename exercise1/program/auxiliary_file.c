...
{
  int idx = col + row * xwidth;
  unsigned char live_neighbors = count_live_neighbors_v2(image, row, col, xwidth, ywidth);
  if (live_neighbors == 2 || live_neighbors == 3){
    image[idx] = 1;
  }
  else if(live_neighbors < 0 || live_neighbors > 8){
...
void static_evolution_v2(char* fname, int n, int s, int argc, char **argv)
{
  ...
  // int cells_per_proc = (xwidth * ywidth) / n_procs; // rows for each process
  // int remaining_cells_proc = (xwidth * ywidth) % n_procs; // remaining rows
  // if (rank < remaining_cells_proc)
  //     remaining_cells_proc++;

  // Allocate auxiliary matrix to evolve the playground
  // Allocate the number of rows to be evolved plus the row above and below
  ...
  if(rank == 1)
  {
    ...
    for(int col = 0; col < xwidth; col++)
    {
      image[col + (row_per_procs + 2 - 1) * xwidth] = original_image[col + (row_per_procs + startrow) * xwidth];
    }
  ...
  #pragma omp parallel
  {
    ...
    const int cells_per_thread = rows_per_thread * xwidth;
    const int remaining_cells_thread = remaining_cells_thread * xwidth;

    // Define the start and end row of each piece of matrix
    int startrow = thread_id * cells_per_thread;
    ...
    for(int step = 0; step < n; step++)
    {
      ...
      for(int row = 1; row < (rows_per_thread + 2 - 1); row++)
      {
        for(int col = 0; col < xwidth; col++)
        {
          check = static_upgrade_v2(unsigned char* image, int xwidth, int ywidth, int row, int col);
      ...
      //   int check = static_upgrade(image, xwidth, ywidth, row, col);
